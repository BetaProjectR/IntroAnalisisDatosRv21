---
title: "Guía clase 06 - Análisis exploratorio de datos"
...
Luego guarde inmediatamente su *script* como **script_06_nombre_apellido.Rmd**. Al finalizar la actividad deberá exportar y almacenar este script en su carpeta drive de tareas.
...
El archivo **"robalo.xlsx"** contiene el registro de genotipos y peso de cuerpo (weight) de 1000 robalos (*Eleginops maclovinus*) colectados en el intermareal de la costa de Chile.

**a)** Elabore un objeto denominado **datos** e importe el set de datos **robalo.xslx** usando la función **read_excel()** de la librería **readxl**. Observe los datos con la función **summary()** y **str**. Note que las variables **id** y **genotipos** están codificadas como **character** y el **weight** como **numeric**

```{r}
datos <- read_excel("robalo.xlsx")
summary(datos)
str(datos)
```
...
**d)** agregue a al código anterior el comando **geom_jitter()** para visualizar todos los datos de "Peso cuerpo (g)"

```{r}
ggplot(datos, aes(x=genotipo, y=weight, fill=genotipo)) +
    geom_boxplot()+
  labs(y="Peso de cuerpo (g)")+
geom_jitter(color="blue", size=1, alpha=2)
```


### **Ejercicio 6.** Explorar y visualizar la interacción de dos factores
El archivo **"dietas.xlsx"** contiene el registro de pesos de 1000 robalos de un experimento de alimentación con 6 dietas diferentes de algas.

**a)** Importe el set de datos **dietas.xslx** usando la función **read_excel()** de la librería **readxl**.
...
dietas$id<- as.factor(dietas$id)
dietas$feed<- as.factor(dietas$feed)
dietas$sex<- as.factor(dietas$sex)
summary(dietas)
table(dietas$feed, dietas$sex)
# Ejecute como prueba pero no incluya en el reporte  table(dietas$feed, dietas$id) 
```

**b)** A partir del set de datos de dietas elabore una gráfica de cajas y bigotes para la variable **weight**. Use el comando **ggplot()** y el argumento **aes(x,y,fill)** para seleccionar los datos, el factor **feed**, la variable respuesta **weight** y el 2do factor **sex**. Agregue el comando **geom_boxplot()** para obtener el boxplot.
...
```{r}
ggplot(dietas, aes(x=feed, y=weight, fill=sex)) +
    geom_boxplot()+
  labs(y="Peso de cosecha (g)")
```
...
```{r}
interaction.plot(dietas$feed, dietas$sex, dietas$weight, fun = mean)
```
...
El archivo **"gonada.xlsx"** contiene el registro de peso del cuerpo y peso de la gónada de 100 hembras de róbalo.

**a)** Importe el set de datos **gonada.xslx** usando la función **read_excel()** de la librería **readxl**. Explore el set de datos completo con la función **summary()**.

```{r}
gonada <- read_excel("gonada.xlsx")
summary(gonada)
```
...
pairs.panels(gonada[,], method = "pearson", hist.col = "red",  density = TRUE, font=2)
```

**c)** Elimine la variable **id** del análisis anterior seleccionando las columnas 2 y 3 (gonada[,2:3]) y responda: 

¿ Que tipo de distribución tienen las variables **weight** y **gonad_weight**?

¿Están correlacionadas las variables **weight** y **gonad_weight**?

```{r}
# Gráfica de correlación de variables continuas
pairs.panels(gonada[,2:3], method = "pearson", hist.col = "red",  density = TRUE, font=2)
```
