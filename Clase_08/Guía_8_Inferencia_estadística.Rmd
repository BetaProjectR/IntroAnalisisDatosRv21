---
title: "Guía 8 Inferencia estadística"
...
Ejemplo: Probar que la Bahía A está más contaminada que la Bahía B.
...
Luego guarde inmediatamente su *script* como **script_8_nombre_apellido.Rmd**. Al finalizar la actividad deberá exportar y entregar su tarea en el navegador académico.
...
Los datos **Robalo_data.xlsx** se refieren a la longitud (**Longitud_cm**) y el peso (**Peso_g**) de 100 robalos provenientes del estero de Reloncaví.

**a).** Importe el set de datos **Robalo_data.xlsx** usando la función **read_excel()** de la librería **readxl**, llame al objeto Robalo. Explore el set de datos usando las funciones **head()**, **str()** y **summary()**.

```{r}
Robalo <- read_excel("Robalo_data.xlsx")
head(Robalo)
str(Robalo)
summary(Robalo)
```
...
Promedio_Longitud <- mean(Robalo$Longitud_cm)
Var_Longitud <- var(Robalo$Longitud_cm)

# Promedio y varianza de la variable Peso
Promedio_Peso <- mean(Robalo$Peso_g)
Var_Peso <- var(Robalo$Peso_g)

# Histogramas de las variables Longitud y Peso
par(mfrow = c(1,2))
hist(Robalo$Longitud_cm, main= "Hist Longitud", xlab= "Longitud (g)", ylim= c(0,30),col="cyan")
hist(Robalo$Peso_g, main= "Hist Peso", xlab= "Peso (g)", ylim= c(0,30),col="dodgerblue")

# Gráfico de dispersión entre x e y
ggplot(Robalo, aes(x=Robalo$Longitud_cm, y=Robalo$Peso_g))+
  geom_point()+
...
```{r}
cor(Robalo$Longitud_cm,Robalo$Peso_g)
```
...
```{r, echo=TRUE}
cor_L_P<- cor.test(x=Robalo$Longitud_cm , y=Robalo$Peso_g , method = "pearson", conf.level = 0.95)
cor_L_P
...
Los datos **Robalos_Machos.xlsx** se refieren el peso (**Peso_g**) de 100 Robalos machos del sur de chile.

**a).** Importe el set de datos **Robalos_Machos.xlsx** usando la función **read_excel()** de la librería **readxl**, llame al objeto Robalos_Machos. Explore el set de datos usando las funciones **head()**, **str()** y **summary()**. 

```{r}
Robalos_Machos <- read_excel("Robalos_Machos.xlsx")
head(Robalos_Machos)
str(Robalos_Machos)
summary(Robalos_Machos)
```

**b).** A partir del set de datos de **Robalos_Machos** elabore un histograma para la variable **Peso**. Use el comando **ggplot()** y el argumento **aes(x)**. Agregue el comando **geom_histogram(color="black", fill="white")** para obtener el histograma y el comando **labs(x="Peso (g)",title="Histograma de la variable Peso(g)")**.
...
**c).**Calcular la media y la varianza del peso de los machos.Luego,realice una tabla los estimadores puntuales del promedio y la varianza de la variable peso llamada Tabla_peso. Muestre la Tabla_peso en formato knitr::kable(), colocar en el parámetro **caption** "Media y varianza de Robalos machos".

```{r}
Tabla_Peso<- Robalos_Machos %>% 
  summarize(Media_M = mean(Peso_Machos),
             Var_M = var(Peso_Machos))

# Tabla con los datos
knitr::kable(Tabla_Peso,caption ="Media y varianza de Robalos machos")
```
...
```{r, echo=TRUE}
DifMed_M <- t.test(x=Robalos_Machos$Peso_Machos,mu = 190, alternative = c("greater"), conf.level = 0.95)
DifMed_M
...
Machos <- Robalos_dat[Robalos_dat$Sexo=="Macho", ]
Prome_Peso_Machos <- mean(Machos$Peso)
Var_Peso_Machos <- var(Machos$Peso)

Hembras <- Robalos_dat[Robalos_dat$Sexo=="Hembra", ]
...
# Tabla con los datos
knitr::kable(Tabla,caption ="Medias y varianzas de Robalos_dat machos y hembras")
```
